<?php
    ...
    if($sth) {
        $stmt = "UPDATE bookings SET approval=2 WHERE id = '$uniq_id'";
        $sth = $conn->prepare($stmt);
        $sth->execute();
        if($sth) {
            ...
            echo "<script>alert('Fail To Delete Appointment')</script>";
        }
    } else {
    ...
    function sendDisapproveEmail($data) {
        ...
        $cMail->Subject = "Reject Slot Booked";
        ...
        if(!$cMail->send()) {
            ...
            echo 'Disapprove Message has been sent';
        }
    }
    ...
    function message($data) {
        ...
        $message .= "</table>";
        $message .= "<br>";
        $message .= "<strong>Slot Booked Above is Rejected. Kindly book another time. Thank You.</strong>";
        $message .= "</body></html>";
    }
